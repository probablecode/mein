...
//  }
//
//  // Ensure that we have exactly 4 octets
//  if (octets.size() != 4) {
//	throw std::runtime_error("Invalid IP address format");
//  }
//
//  // Convert each octet to an integer and reverse the order
//  std::reverse(octets.begin(), octets.end());
//
//  uint32_t value = 0;
//  for (size_t i = 0; i < octets.size(); ++i) {
//	char* endPtr;
//	long int octetValue = std::strtol(octets[i].c_str(), &endPtr, 10);
//	if (*endPtr != '\0' || octetValue < 0 || octetValue > 255) {
//	  throw std::runtime_error("Invalid IP address format");
//	}
...
Server createServer(std::string& listen) {
  ...
  return Server(inet_addr(ip.c_str()), portToInt(port));
}
